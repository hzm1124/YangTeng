...
- |网址|爬前操作|目录字段|内容字段|爬取方法|
  ...
  |08. [SpectraPremium](https://ecat.spectrapremium.com/en/parts?line=oil_pans&year=&make=&model=&submodel=&universal=0&hide-exclusives-canadian-market=0&sort=part-number&limit=50)|1. `Sort by part #`<br />2. `Number of results` -> `50`|`Menu`、***`Url`***|1. `$_序号`、`Part_Number`、`$_Vehicle`、***`Url`***、`Src`、`其他Part_Specifications`<br />2. ***`Url`***、`Vehicle`|requests|
