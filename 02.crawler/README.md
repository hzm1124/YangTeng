...
- |网址|爬前操作|目录字段|内容字段|爬取方法|
  ...
  |04. [Cardone](https://www.cardone.com/motors/wiper-and-washer/windshield-wiper-motor/?limit=96&sort=alphaasc)|1. `Show` -> `96`<br />2. `Sort By:` -> `A to Z`|***`Part_Number`***、`Url`、`Application_Summary`|1. `$_序号`、`Part_Type`、***`Part_Number`***、`Title`、`Application_Summary`、`Vehicle`、`OE`、`$_Pic`、`$_Url`、`$_Src`、`其他General`、`其他Technical`|requests + Tomcat|
  ...
  |17. [Mevotech](https://www.mevotech.com/part/CMK100003)|从`RockAuto`爬下来`Part_Number`，再去`Mevotech`官网获取`车型`和`位置`信息||`Part_Number`、`Vehicle`|requests|
